      ...
      <div className="flex items-center justify-between max-w-7xl mx-auto gap-1.5 sm:gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          <motion.button
            ...
            disabled={isSyncing}
            className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 md:py-2 rounded-xl border text-xs font-semibold transition-all ${
              syncStatus === 'success'
                ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-300'
                : 'bg-zinc-900/90 border-white/10 text-zinc-300 hover:text-white hover:border-blue-500/40'
            }`}
            title="Sync all data with Supabase cloud database"
          >
            {isSyncing ? (
            ...
            <span className="hidden sm:inline">
              {isSyncing ? 'Syncing...' : syncStatus === 'success' ? 'Synced!' : 'Sync Cloud'}
            </span>
          </motion.button>
